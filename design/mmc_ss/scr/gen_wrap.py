...
def get_parse():
  parser = argparse.ArgumentParser(description='''Instance design and generate the wrap,
if there are both input and output port with the same name, then they are connected,
otherwise, the port are exported to wrap port.
  ''')
  parser.add_argument('-i', '--design' , metavar='design.v'     , nargs='+', default='a.v', help='input file'    )
  parser.add_argument('-o', '--wrapper', metavar='design_wrap.v', nargs='+', default='c.v', help='output file'   )
...
def inst_port(module_dict, gen_design):
  ...
  for module_name in module_dict:
    ...
    for port in module_port.keys():
      port_attri   = module_port[port]
      if len(port_attri[4]) < 1 :
        inst_port    = '\n\t' + port_attri[0] + '\t' + port_attri[1]  + '\t' + port_attri[3] + '\t'
...
def inst_module(module_dict):
  ...
  for module_name in module_dict:
    ...
    for port in module_port.keys()[0:-1]:
      port_attri   = module_port[port]
      inst_port    = '\n\t\t.' + port + '\t(' + port_attri[3] + '\t),\t//\t' + str(port_attri[4])
